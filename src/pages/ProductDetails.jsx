...
export default function ProductDetails({ onAddToCart, onToggleFavorite, favorites }) {
  ...
  return <main className="detail-page"><div className="container"><Link className="back-link" to="/products"><ArrowLeft size={15} /> Back to collection</Link><div className="detail-layout"><div className="detail-image"><img src={product.image} alt={product.title} /><span>{product.category}</span></div><div className="detail-copy"><p className="eyebrow">{product.brand}</p><h1>{product.title}</h1><div className="detail-rating"><Star size={15} fill="currentColor" /> {product.rating} <span>·</span> 24 reviews</div><p className="detail-description">{product.description}</p><div className="detail-price">${product.price.toLocaleString('en-US')}</div><p className="stock"><span></span> In stock · ships within 24 hours</p><div className="detail-actions"><button className="button button-dark" onClick={() => onAddToCart(product)}>Add to cart <Plus size={16} /></button><button className={`favorite-detail ${isFavorite ? 'is-favorite' : ''}`} onClick={() => onToggleFavorite(product.id)}><Heart size={18} fill={isFavorite ? 'currentColor' : 'none'} /> {isFavorite ? 'Saved' : 'Save'}</button></div><div className="detail-perks"><div><Truck size={19} /><span><strong>Free delivery</strong><small>On orders over $75</small></span></div><div><ShieldCheck size={19} /><span><strong>30-day returns</strong><small>Easy, no-questions returns</small></span></div></div></div></div></div></main>
}